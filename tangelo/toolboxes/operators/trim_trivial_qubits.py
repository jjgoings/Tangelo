...
def trim_trivial_operator(qu_op, n_qubits, trim_states, trim_index, reindex=True):
    """
    Calculates expectation values of a QubitOperator acting on qubits in a 
    trivial |0> or |1> state, a returns a trimmed QubitOperator with updated coefficients
    
    Args:
        qu_op (QubitOperator): Operator to trim
        n_qubits (int): number of qubits in full system
        trim_state(list): state of the qubits to trim, 0 or 1
...
def trim_trivial_circuit(circuit, reindex=True):
    """ 
        Splits Circuit into entangled. and unentangled components. 
        Returns entangled Circuit, and the index and state of unentangled qubits 
        ...
        Returns:
            ...
            trim_state(list): state of removed qubits, 0 or 1
    ...
    zeros = list(sorted(set(indices) - set(gated_qubits)))   
...
def trim_trivial_qubits(operator, circuit, reindex=True):
    """ 
    ...
    """
    trimmed_circuit, trim_index, trim_states = trim_trivial_circuit(circuit, reindex)
    trimmed_operator = trim_trivial_operator(operator, circuit.width, trim_states, trim_index, reindex=True)
